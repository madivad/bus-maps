# app.py  -- this is an edit to include caching, can be applied immediately as no changes have been made yet and this fits, but I don't like the implementation (yet), I would like to cache the database entirely, not by "sets" of routes, as not may people/users will have the same set.
import os
import csv
...
def load_gtfs_shapes(target_realtime_routes: set):
    ...
    # Create a cache key from the frozenset of target routes
    # frozenset is used because sets are mutable and thus not hashable for dict keys
    ...
    if not target_short_names:
        ...
        _route_shapes_cache[cache_key] = {} # Cache empty result
        return {}
        ...
        for static_id in static_ids_for_key:
            ...
            for shape_id in shape_ids_for_static_route:
                if shape_id in processed_shape_points:
                    ...
                    # Convert list of dicts to tuple of tuples of items to make it hashable for the set
                    point_tuple = tuple(tuple(sorted(p.items())) for p in point_list) # sorted items for consistent hashing
                    shapes_for_this_realtime_id.add(point_tuple)

        if shapes_for_this_realtime_id:
             # Convert back from tuple of tuples of items to list of dicts for JSON response
    ...
    final_result = dict(route_shapes_for_this_request) # Convert back to dict from defaultdict
    _route_shapes_cache[cache_key] = final_result # Store result in cache
