...
def fetch_and_filter_bus_positions(api_url, api_key, target_routes_set):
    """
    ...
    Args:
        ...
        target_routes_set (set): A set of route_id strings to filter for.
                                 This set comes from the API request now.
    ...
    if not api_key:
        ...
        return None
    if not api_url:
         print("Error: API URL is missing (check environment variables).")
         return None

    # If the target_routes_set is empty, no need to fetch data
    if not target_routes_set:
         # print("No target routes provided for fetch. Returning empty list.") # Less noisy
         return []
    ...
    # print(f"Fetching data from {api_url} for routes: {', '.join(target_routes_set)}...") # Too verbose

    response = None # Initialize response to None
    try:
        response = requests.get(api_url, headers=headers, timeout=30) # 30 second timeout
        response.raise_for_status()  # Raise an exception for bad status codes (4xx or 5xx)
        # print("Data fetched successfully.") # Less noisy
        ...
        if response is not None: # Check if response object exists
             if response.status_code == 401 or response.status_code == 403:
                  ...
                  print("API endpoint not found (404). Check the URL (BUS_URL in .env).")
        return None
    ...
    try:
        ...
        # print("Data parsed successfully.") # Less noisy
    ...
    for entity in feed.entity:
        ...
        if entity.HasField('vehicle') and entity.vehicle.HasField('trip'):
            ...
            if trip.HasField('route_id') and str(trip.route_id) in target_routes_set:
                ...
                if vehicle.HasField('position') and vehicle.position.HasField('speed'):
                    try:
                        ...
                        # print(f"Warning: Error calculating speed for vehicle {vehicle.vehicle.id if vehicle.HasField('vehicle') else 'Unknown'}: {e}") # Too noisy
                ...
                position_info = {
                    ...
                    "raw_timestamp": vehicle.timestamp if vehicle.HasField('timestamp') else None
                }
                matching_vehicles.append(position_info)

    # print(f"Found {len(matching_vehicles)} vehicles matching the requested routes.") # Less noisy
